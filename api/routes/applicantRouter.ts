...
applicantRouter.post('/', auth, imagesUpload.single('photo'), async (req: RequestWithUser, res, next) => {
  try {
    const userId = req.query.userId;
    const {
      firstName,
...
applicantRouter.delete('/:id', auth, permit('admin', 'superadmin'), async (req: RequestWithUser, res, next) => {
  try {
...
applicantRouter.patch('/:id/activate', auth, permit('admin', 'superadmin'), async (req: RequestWithUser, res, next) => {
  try {
