...
const EmployerCard: React.FC<EmployerCardProps> = ({ data }) => {
  return (
    <div className="EmployerCard">
      <Avatar src={data.image.url ? data.image.url : noImage} alt={data.emplinfo[0].text} sx={{height: '130px', width: '130px'}} />
      <Typography variant="h6" sx={{width: '50%', mt: '10px'}}>{data.emplinfo[0].text}</Typography>
