...
const BlockForm: React.FC<Props> = ({ block, onSubmit, loading }) => {
    ...
    if (block) {
      setCardForm(
        block.cards.map((card) => (
          <CardForm key={card.id} card={card} cardId={card.id} addCard={addCardToBlockState} deleteCard={deleteCard} />
        )),
      );
    }
  }, [block]);
  ...
  const addCardToBlockState = (newCard: Card) => {
    ...
    if (existingCardId !== -1) {
      setState((prevState) => ({
        ...prevState,
        cards: prevState.cards.map((card, index) => (index === existingCardId ? newCard : card)),
      }));
    } else {
      setState((prevState) => ({
        ...prevState,
  ...
  const addCardForm = () => {
    ...
    setCardForm((prevState) => [
      ...prevState,
      <CardForm key={cardId} card={null} cardId={cardId} addCard={addCardToBlockState} deleteCard={deleteCard} />,
    ]);
  };
    ...
    <div>
      <form autoComplete="off" onSubmit={submitFormHandler}>
        <Grid container direction="column" spacing={2}>
          ...
          <Grid item xs>
            <Button sx={{ marginTop: '10px' }} onClick={() => addCardForm()}>
              Добавить карточку
              <AddIcon />
            </Button>
          </Grid>
